...
def test_static_index_html_117_ids_strict():
    ...
    assert len(parser.ids) == 117, f"Expected exactly 117 static IDs, found {len(parser.ids)}"
    assert len(set(parser.ids)) == 117, f"Duplicate IDs detected: {[i for i in parser.ids if parser.ids.count(i) > 1]}"
