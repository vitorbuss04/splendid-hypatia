...
class TestTier1FeatureCoverage:
    ...
    def test_t1_dom_117_ids_strict_preservation(self):
        ...
        assert len(found_ids) >= 117, f"Expected at least 117 static IDs, found {len(found_ids)}"
        assert len(set(found_ids)) == len(found_ids), "All element IDs must be unique (no duplicates)"
    ...
    def test_t1_projects_view_and_search_filter(self):
        ...
        assert search_input.get("type") in ["text", "search"]
